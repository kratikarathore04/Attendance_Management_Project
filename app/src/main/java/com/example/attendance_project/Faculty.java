...
public class Faculty extends AppCompatActivity implements  AdapterView.OnItemClickListener {
    ...
    public void onItemClick(AdapterView<?> a, View v, int p, long i){
        full_detail = l1.getItemAtPosition(p).toString();//konsa item mila hai p psoition pe vo save kr liya
        final Dialog dialog = new Dialog(this);//object bnaya hai
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);//...........
        dialog.setContentView(R.layout.test_layout);//custom dialogue will show  test layout is a design
// set title and cancelable
        RadioGroup radioGroup;//SIRF EK HI BUTTON SELECT KR SKTE HAI
        RadioButton present;
        RadioButton absent;
        radioGroup = (RadioGroup) dialog.findViewById(R.id.radioGroup);
        present=(RadioButton)dialog.findViewById(R.id.PresentradioButton);//DIALOGUE KE THROUGH ID
        absent=(RadioButton)dialog.findViewById(R.id.AbsentradioButton);
        radioGroup.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            @Override//EVENT HANDELLING CODE
